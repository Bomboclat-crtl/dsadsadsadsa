...
function imagenAuto(nombre) {
  const texto = encodeURIComponent(nombre);
  return `https://placehold.co/600x400/eaf5df/2f5d3a?text=${texto}`;
}
  ...
  filtrados.forEach(p => {
    ...
    col.innerHTML = `
      <div class="card h-100 shadow-sm border-0" style="border-radius:1rem; overflow:hidden;">
        <img src="${imagenAuto(p.nombre)}" class="card-img-top" alt="${p.nombre}" style="height:160px; object-fit:cover;">
        <div class="card-body d-flex flex-column">
          <span class="badge text-bg-success mb-2 align-self-start">${categorias[p.categoria]}</span>
          <h5 class="card-title fw-bold">${p.nombre}</h5>
          <p class="card-text text-secondary small">${p.descripcion}</p>
          <p class="fw-bold mb-3 text-success">S/ ${p.precio.toFixed(2)}</p>
          <button class="btn btn-success mt-auto" type="button">Comprar</button>
        </div>
      </div>
    `;
